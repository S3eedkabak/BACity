...
| 0. Accounts | Email verification and reset with hashed single-use tokens; durable SMTP outbox/retries; JWT expiration and token-version revocation; logout; account anonymization; blocking; role checks; first-admin CLI | Configure SMTP or local Mailpit; complete deletion/export retention policy across all historical records and provider data; operational recovery procedures |
...
| 3. Profiles/follows | Profile/privacy API and settings; public contribution metadata; people search; follow users/guides/venues/organizers/neighborhoods/categories; blocking | Avatar upload/editor and richer public profiles; full follow-target selection UI; pagination UX |
...
| 6. Utilities | Extensible utility kinds, location, hours, fee/accessibility/changing/cleanliness/status fields; moderated submission; daily confirmation limit; filters; map links | Native map layers and viewport/nearby utility queries, imported city utility dataset, freshness/confidence aggregation across confirmations |
