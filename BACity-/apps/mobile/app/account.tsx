...
export default function Account() {
  ...
  return <Page title="Your account">
    ...
    {!user ? <Card><Field label="Email address" value={email} onChange={setEmail} /><Button busy={busy} title="Send password reset email" onPress={() => run(() => apiRequest('/auth/request-reset', { method: 'POST', body: { email } }), 'If the account exists, a reset email has been queued.')} /><Button title="Sign in" onPress={() => router.push({ pathname: '/auth', params: { mode: 'login' } })} /></Card> : <>
      ...
      <Card><Text style={ui.heading}>Profile photo</Text><Text style={ui.text}>Choose a photo, then crop and position it in the square editor.</Text><Button title="Choose and crop photo" busy={busy} onPress={chooseAvatar} /></Card>
