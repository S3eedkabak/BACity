...
import { Card, Field, Button, Notice, ui } from './CommunityUI';
...
export function Discussion({ id, kind }: { id: string; kind: 'event' | 'place' }) {
  ...
  if (!token) return <Card><Text style={ui.text}>Sign in to read reviews and join the discussion.</Text><Button title="Sign in" onPress={() => router.push('/(tabs)/profile')} /></Card>;
