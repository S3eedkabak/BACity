...
export function Discussion({ id, kind }: { id: string; kind: 'event' | 'place' }) {
  ...
  return <View style={{ gap: 16, padding: 14 }}><Notice text={notice} />
    ...
    <Card><Button title="Suggest a correction" onPress={() => router.push({ pathname: "/correction", params: { type: kind, id } })} /><Button title="Create a collection with this item" onPress={() => router.push({ pathname: "/collection", params: { type: kind, id } })} /><Field label="Report reason" value={reason} onChange={setReason} multiline /><Button title={'Report this ' + kind} busy={busy} onPress={() => act('/community/reports', { target_type: kind, target_id: id, reason })} /><Button title="Refresh discussion" onPress={load} /></Card>
  </View>;
}
